...
var MethodLibrary = (function () {
  return {
     ...
     * 创建loading
     */
    createLoading: function (text: string, showTimeCount: number) {
      var count = 1
      var timeCountStr = ''
      if (showTimeCount) {
        timeCountStr =
      ...
      if (text) {
        textStr =
          '<div style="padding: 5px 10px;border-radius: 3px;color:white;background-color: #28a745;font-size: 16px;font-weight: 300;">' +
          text +
          '</div>'
      }
    ...
    // 获取匹配库
    getMatchMap: function (u: string | string[]) {
      return {
        // 内核
    ...
    // 在信息map和匹配库中进行匹配
    matchInfoMap: function (_this: any) {
      var u = (VariableLibrary.navigator as any).userAgent || {}
      var match = MethodLibrary.getMatchMap(u)
      for (var s in VariableLibrary.infoMap) {
        for (var i = 0; i < (VariableLibrary.infoMap as any)[s].length; i++) {
          var value = (VariableLibrary.infoMap as any)[s][i]
    ...
    getLanguage: function () {
      ...
      _this.language = (function () {
        var language =
          VariableLibrary.navigator.browserLanguage || VariableLibrary.navigator.language
    ...
    getBrowserInfo: function () {
      ...
      var _mime = function (option: string, value: string) {
      ...
      if ((window as any).chrome) {
        ...
        if (chrome_version > 36 && (window as any).showModalDialog) {
          is360 = true
      ...
      var browerVersionMap: any = {
        ...
        '360SE': function () {
          var hash: any = { 63: '10.0', 55: '9.1', 45: '8.1', 42: '8.0', 31: '7.0', 21: '6.3' }
          var chrome_version = u.replace(/^.*Chrome\/([\d]+).*$/, '$1')
          return hash[chrome_version] || ''
        },
        '360EE': function () {
          var hash: any = { 69: '11.0', 63: '9.5', 55: '9.0', 50: '8.7', 30: '7.5' }
          var chrome_version = u.replace(/^.*Chrome\/([\d]+).*$/, '$1')
          return hash[chrome_version] || ''
        },
        Maxthon: function () {
        ...
        LBBROWSER: function () {
          ...
          var chrome_version = u.replace(/^.*Chrome\/([\d]+).*$/, '$1')
          return version || (hash as any)[chrome_version] || ''
        },
    ...
    // 获取地理位置
    getGeoPostion: function (callback: (arg0: GeolocationPosition) => void) {
      navigator &&
...
var LogicLibrary = (function () {
  return {
    DeviceInfoObj: async (params: { info: any[] }, callback: (arg0: {}) => void) => {
      var info = {
        ...
        // fingerprint: await MethodLibrary.createFingerprint(params.domain), // 浏览器指纹
        ...
        for (var i in info) {
          params.info.forEach(function (item: string) {
            if (item.toLowerCase() === i.toLowerCase()) {
              item = i
              infoTemp[item] = (info as any)[item]
            }
          })
        }
        resultInfo = infoTemp
...
export const getDeviceInfo = (params: any, callback: (arg0: any) => void) => {
  LogicLibrary.DeviceInfoObj(params, function (data: any) {
    callback(data)
  })
}
